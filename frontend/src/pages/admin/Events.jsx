...
const Events = () => {
  ...
  const [formData, setFormData] = useState({ name: '', venueId: '' });
  ...
  const fetchEvents = async () => {
    setLoading(true);
    try {
      const response = await eventAPI.getAll({ page, limit });
  ...
  const fetchVenues = async () => {
    try {
      const response = await venueAPI.getAll({ page: 1, limit: 100 });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {events.map((event) => (
                <div key={event.id} className="bg-white rounded-lg shadow-md p-6">
                  ...
                  <p className="text-gray-600">Venue: {event.venue?.name || 'N/A'}</p>
